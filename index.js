...
function nextNewYear() {
  ...
  const newYearDay = new Date(thisYear + 1, 0, 1);
  const diff = newYearDay - currentDate;
...
function addDays(date, days) {
  ...
  const addedDate = date.setDate(date.getDate() + days);
  const resultDate = date.toISOString(addedDate);
  return {
    inputDate: inputDate,
    addedDays: days,
...
function getDaysInMonth(date) {
  ...
  nextMonthFirstDay.setFullYear(inputYear, inputMonth + 1, 1);
  const lastDayOfInputMonth = new Date(nextMonthFirstDay);
  lastDayOfInputMonth.setDate(lastDayOfInputMonth.getDate() - 1);
  const daysInMonth = lastDayOfInputMonth.getDate();
  return {
    inputDate,
    daysInMonth,
  };
...
function getDaysDifference(startDate, endDate) {
  ...
  const startDayISO = startDate.toISOString();
  const endDayISO = endDate.toISOString();
  return {
    startDate: startDayISO,
    endDate: endDayISO,
